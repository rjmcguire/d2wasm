/**
 * WebAssembly Binary Format - Core Types and Encoding
 * 
...
enum BlockType : ubyte {
    void_ = 0x40,  // Empty block type (no result)
    i32 = 0x7F,
    i64 = 0x7E,
    f32 = 0x7D,
    f64 = 0x7C,
 ...
 */
void leb128u(ref Appender!(ubyte[]) out_, ulong value) {
    do {
 ...
 */
void leb128s(ref Appender!(ubyte[]) out_, long value) {
    bool more = true;
    while (more) {
        ubyte b = cast(ubyte)(value & 0x7F);
        value >>= 7;
...
/**
 * Convenience: encode unsigned and return bytes
 */
ubyte[] encodeLEB128u(ulong value) {
    Appender!(ubyte[]) out_;
    leb128u(out_, value);
    return out_.data;
}
...
/**
 * Convenience: encode signed and return bytes
 */
ubyte[] encodeLEB128s(long value) {
    Appender!(ubyte[]) out_;
    leb128s(out_, value);
    return out_.data;
}
...
unittest {
    ...
    assert(encodeLEB128u(0) == [0x00]);
    assert(encodeLEB128u(1) == [0x01]);
    assert(encodeLEB128u(127) == [0x7F]);
    assert(encodeLEB128u(128) == [0x80, 0x01]);
    assert(encodeLEB128u(624485) == [0xE5, 0x8E, 0x26]);
    
    // Test signed LEB128
    assert(encodeLEB128s(0) == [0x00]);
    assert(encodeLEB128s(1) == [0x01]);
    assert(encodeLEB128s(-1) == [0x7F]);
    assert(encodeLEB128s(63) == [0x3F]);
    assert(encodeLEB128s(-64) == [0x40]);
    assert(encodeLEB128s(64) == [0xC0, 0x00]);
    assert(encodeLEB128s(-65) == [0xBF, 0x7F]);
}
