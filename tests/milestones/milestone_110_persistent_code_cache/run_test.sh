...
if [ "$HITS1" != "0" ] || [ "$MISSES1" != "11" ]; then
    echo "FAIL: First compile should have 0 hits, 11 misses. Got hits=$HITS1 misses=$MISSES1"
    exit 1
fi
echo "  OK: 0 hits, 11 misses"
...
if [ "$HITS2" != "11" ] || [ "$MISSES2" != "0" ]; then
    echo "FAIL: Second compile should have 11 hits, 0 misses. Got hits=$HITS2 misses=$MISSES2"
    exit 1
fi
echo "  OK: 11 hits, 0 misses"
...
if [ "$HITS3" != "9" ] || [ "$MISSES3" != "2" ]; then
    echo "FAIL: Modified source should have 9 hits, 2 misses. Got hits=$HITS3 misses=$MISSES3"
    exit 1
fi
echo "  OK: 9 hits, 2 misses"
