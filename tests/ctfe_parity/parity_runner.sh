#!/bin/bash
#
# CTFE Parity Test Runner
# Runs each test with ALL CTFE backends to ensure feature parity
#
...
# CTFE backends that must have feature parity
CTFE_BACKENDS=(wasm native)
    ...
    case "$test_type" in
        compile_output)
            ...
            if echo "$compile_output" | grep -qF "$expected_output"; then
                echo -e "${GREEN}PASS${NC} $test_name [$backend] (output: $expected_output)"
                return 0
            else
                echo -e "${RED}FAIL${NC} $test_name [$backend]"
...
run_test_all_backends() {
    ...
    for backend in "${CTFE_BACKENDS[@]}"; do
...
echo -e "${CYAN}CTFE Parity Test Runner${NC}"
echo -e "Backends: ${CTFE_BACKENDS[*]}"
echo
...
echo
echo -e "${CYAN}CTFE Parity Results:${NC} $passed passed, $failed failed"
echo "Backends tested: ${CTFE_BACKENDS[*]}"
