(module
  ;; Import WASI fd_write function
  (import "wasi_snapshot_preview1" "fd_write" (func $fd_write (param i32 i32 i32 i32) (result i32)))
  
  ;; Memory for string data and iovec structures
  (memory 1)
  
  ;; String data: "Hello from D!\n"
  (data (i32.const 1024) "Hello from D!\0A")
  (data (i32.const 1040) "42\0A")
  
  ;; Memory offset tracker
  (global $memory_offset (mut i32) (i32.const 2048))
  
  ;; Function to write a string using WASI fd_write
  (func $writeln_string (param $ptr i32) (param $len i32)
    (local $iovec_ptr i32)
    (local $nwritten_ptr i32)
    
    ;; Get memory for iovec (8 bytes)
    global.get $memory_offset
    local.set $iovec_ptr
    
    ;; iovec.iov_base = string pointer
    local.get $iovec_ptr
    local.get $ptr
    i32.store
    
    ;; iovec.iov_len = string length
    local.get $iovec_ptr
    i32.const 4
    i32.add
    local.get $len
    i32.store
    
    ;; Get memory for nwritten (4 bytes)
    global.get $memory_offset
    i32.const 8
    i32.add
    local.set $nwritten_ptr
    
    ;; Call fd_write(stdout=1, iovec_ptr, iovec_count=1, nwritten_ptr)
    i32.const 1  ;; stdout file descriptor
    local.get $iovec_ptr
    i32.const 1  ;; iovec count
    local.get $nwritten_ptr
    call $fd_write
    drop  ;; ignore return value
    
    ;; Update memory offset
    global.get $memory_offset
    i32.const 12
    i32.add
    global.set $memory_offset
  )
  
  ;; Main function that demonstrates writeln
  (func $main (result i32)
    ;; writeln("Hello from D!")
    i32.const 1024  ;; string pointer
    i32.const 15    ;; string length (including newline)
    call $writeln_string
    
    ;; writeln(42)
    i32.const 1040  ;; "42\n" pointer
    i32.const 3     ;; length
    call $writeln_string
    
    ;; return 0
    i32.const 0
  )
  
  (export "main" (func $main))
)